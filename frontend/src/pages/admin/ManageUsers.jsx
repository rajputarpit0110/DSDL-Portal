      ...
      <Card style={{ overflow: 'hidden' }}>
        <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--color-border)', display: 'flex', gap: '1rem' }}>
          <div style={{ position: 'relative', flex: 1, maxWidth: '400px' }}>
            ...
            <input type="text" placeholder="Search users by name or email..." style={{ width: '100%', padding: '0.75rem 1rem 0.75rem 2.5rem', borderRadius: '8px', border: '1px solid var(--color-border)', outline: 'none' }} />
          </div>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            ...
            <tbody>
              {mockMembers.map((member, i) => (
                <tr key={member.id} style={{ borderBottom: i === mockMembers.length - 1 ? 'none' : '1px solid var(--color-border)' }}>
                  <td style={{ padding: '1rem 1.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      <div style={{ width: '32px', height: '32px', borderRadius: '50%', backgroundColor: '#e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748b', fontSize: '0.875rem' }}>
                  ...
                  <td style={{ padding: '1rem 1.5rem' }}>
                    <Badge color={member.role === 'Domain Lead' ? 'var(--color-primary)' : '#64748b'}>{member.role}</Badge>
                  </td>
                  <td style={{ padding: '1rem 1.5rem', color: 'var(--color-text-main)' }}>{member.domain}</td>
                  <td style={{ padding: '1rem 1.5rem' }}>
